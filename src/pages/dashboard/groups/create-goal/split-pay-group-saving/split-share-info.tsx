...
export const SplitShareInfo: React.FC<{
...
}> = ({ goalDetails, transactions, goalMembers, primaryDetails, paginator, isGettingTransactions }) => {
  ...
  const renderContributions = (
    <>
      {contributions.length === 0 ? (
        <Flash>No Contributions found.</Flash>
      ) : (
        contributions.map((contribution, idx) => {
          return <GroupTransactionRow key={idx} transaction={contribution} />;
        })
      )}
    </>
  );
  ...
  const renderWithdrawals = (
    <>
      {withdrawals.length === 0 ? (
        <Flash>No Wthdrawals found.</Flash>
      ) : (
        withdrawals.map((contribution, idx) => {
          return <GroupTransactionRow key={idx} transaction={contribution} />;
        })
      )}
    </>
  ...
  return (
    <>
      ...
      <div className="w-full">
        <GroupActionButtons
          goalPage={GroupGoalPages.SPLIT_AND_SHARE}
        ...
        <div className="relative flex justify-between">
          <div className="border-b border-neutral-100 overflow-x-scroll lg:overflow-x-hidden w-[800px] lg:w-full">
            <Tab.List className="space-x-8 lg:space-x-16 flex w-[800px] lg:w-full">
              ...
              <Tab>{({ selected }) => <TabHeader selected={selected}>Contributions</TabHeader>}</Tab>
        ...
        <div className="w-full overflow-x-scroll lg:overflow-auto">
          <div className="overflow-x-scroll lg:overflow-x-auto w-[800px] lg:w-full">
            <Tab.Panels>
              ...
              <Tab.Panel className=" flex flex-col space-y-4">{renderContributions}</Tab.Panel>
              <Tab.Panel className=" flex flex-col space-y-4">{renderWithdrawals}</Tab.Panel>
            </Tab.Panels>
          </div>
        </div>
      </Tab.Group>
